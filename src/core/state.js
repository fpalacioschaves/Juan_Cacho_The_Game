...
const DEFAULT_DATA={currentScene:null,inventory:[],selectedItem:null,flags:[]}
export const State={
  data:{...DEFAULT_DATA},
  ...
  setFlag(id){if(!Array.isArray(this.data.flags))this.data.flags=[];if(!this.hasFlag(id))this.data.flags.push(id);this.persist()},
  clearFlag(id){if(!Array.isArray(this.data.flags))return;this.data.flags=this.data.flags.filter(f=>f!==id);this.persist()},
  reset(){this.data={...DEFAULT_DATA};try{Storage.clear()}catch(e){};this.persist()},
  persist(){Storage.save(this.data)},
  load(){const s=Storage.load();if(s&&typeof s==='object'){this.data={...DEFAULT_DATA,...s}}}
}